...
stones_new_analysis <-
    ...
    add_audio_analysis()
pop_music <-
    stones_old_analysis %>% mutate(Released = "60s/70s") %>%
    bind_rows(stones_new_analysis %>% mutate(Released = "2010s"))
```
...
### Looking at Timbre Coefficent between the old and new albums
```{r}
pop_music %>% 
    mutate(
        timbre =
            map(
                ...
                timbre,
                method = 'mean')) %>%
    select(Released, timbre) %>% 
    compmus_gather_timbre %>% 
    ggplot(aes(x = basis, y = value, fill = Released)) +
    geom_violin() +
    theme_max ()+
    scale_fill_viridis_d() +
    labs(x = 'Spotify Timbre Coefficients', y = '', fill = 'Released')
```
